package com.flaremars.processors.db;

import com.flaremars.annotations.Column;
import com.flaremars.annotations.Entity;
import com.flaremars.processors.ProcessingException;
...
public class StorageClass {
    ...
    private TypeSpec.Builder writeSingleInstance(String className) {
        MethodSpec privateConstructor = MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build();
            ...
            if (entry.getKey().equals("id")) {
                ...
                builder.beginControlFlow("if ($L == null)", entry.getKey()).
                        ...
                        addStatement("$T<$L> temp = DataSupport.where($S, $L).find($L.class)",
                                ClassName.get("java.util", "List"), objectClassFullName, entry.getKey() + " == ?", entry.getKey(), objectClassFullName).
